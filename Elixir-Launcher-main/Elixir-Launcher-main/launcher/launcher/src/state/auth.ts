...
export interface AuthState {
  ...
  common_core: AuthResponse["common_core"];
...
const DEFAULT_COMMON_CORE_STRING = '{"VBucks":0}';
...
const useAuth = create<AuthState>((set, get) => ({
  ...
  common_core: JSON.parse(
    localStorage.getItem("auth.common_core") || DEFAULT_COMMON_CORE_STRING
  ),
  login: async (code: string) => {
    ...
    localStorage.setItem(
      "auth.common_core",
      JSON.stringify(r.data.common_core)
    );
  ...
  logout: () => {
    ...
    localStorage.removeItem("auth.common_core");

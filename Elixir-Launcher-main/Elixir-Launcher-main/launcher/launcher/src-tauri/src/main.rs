...
#[tauri::command]
fn rich_presence() {
    let client = DeclarativeDiscordIpcClient::new("1149265231483240448");

    client.enable();

    let buttons = vec![Button::new(
        String::from("Play Elixir!"),
        String::from("https://discord.com/channels/1130704742113357834/1130708532803674152"),
    )];
...
#[tauri::command]
fn delete_pak(path: &str, file_name: &str) -> Result<bool, String> {
    ...
    match fs::remove_file(&file) {
        Ok(_) => {
            println!("The file: {} was deleted successfully", file.display());
            Ok(true)
        }
        Err(e) => {
            eprintln!("Error while deleting {}: {}", file.display(), e);
            Err(format!("Error while deleting {}: {}", file.display(), e))
        }
    }
}
